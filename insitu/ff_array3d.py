...
# field.plot_scene()
# field.monopole_ff()
field.planewave_ff(theta = np.deg2rad(90), phi = np.deg2rad(-60))
...
ff_ded_parray.pk_tikhonov(method = 'scipy', lambd_value=[])
ff_ded_parray.plot_pk_sphere(freq=1000, db=False)
ff_ded_parray.plot_pk_sphere(freq=1000, db=True, dinrange=30, save=False)
ff_ded_parray.pk_interpolate()
ff_ded_parray.plot_pk_map(freq=1000, db=True, dinrange=30, save=False)
# ff_ded_parray.plot_pk_sphere_interp(freq=1000, db=True, dinrange=30, save=False)
# ff_ded_parray.pk_cs(method = 'vcx')
# ff_ded_parray.plot_pk_sphere(freq=1000, db=False)
# ff_ded_parray.plot_pk_sphere(freq=1000, db=True, dinrange=30, save=False)
