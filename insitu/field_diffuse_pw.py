...
class PWDifField(object):
    ...
    def p_fps(self, resistivity = 25000, thickness = 0.1, thick2=0, locally = True, randomize = True, seed = 0):
        ...
        Vp = np.zeros((len(self.sources.coord), len(self.controls.freq)), dtype = complex)
        # np.random.seed(seed)
        ...
        bar = ChargingBar('Calculating sound diffuse pressure at each receiver', max=len(self.receivers.coord), suffix='%(percent)d%%')
        for jrec, r_coord in enumerate(self.receivers.coord):
            ...
            for jf, k0 in enumerate(self.controls.k0):
                ...
                k_vecr[:,2] = -k_vecr[:,2]
                pres_rec[jrec, jf] = np.sum(q * (np.exp(1j * np.dot(k_veci, r_coord))+\
                Vp[:,jf] * np.exp(1j * np.dot(k_vecr, r_coord))))
            bar.next()
        bar.finish()
