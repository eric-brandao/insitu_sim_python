...
theta = np.deg2rad(0) # Angle of incidence in degrees
...
material.layer_over_rigid(thickness = 0.1,theta = theta)
...
# receivers.random_3d_array(x_len = 0.6, y_len = 0.8, z_len = 0.21, n_total= 192, zr = 0.1)

# # receivers.planar_array(x_len=.5, y_len=0.5, zr=0.1, n_x = 8, n_y = 8)
#%% step 7 - run/load field
### to run
field = PWField(air, controls, material, receivers)
# field.p_fps(theta = theta, phi = phi)
# field.plot_scene()
...
file2load = 'pk_inf_2parray_theta45_d25mm' # File you want to load
theta = np.deg2rad(45)
