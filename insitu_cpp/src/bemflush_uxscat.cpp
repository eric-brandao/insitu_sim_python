...
std::complex<float> bemflush_uxscat(Eigen::RowVector3f &r_coord,
    ...
    for (int j = 0; j < Nel; j++){
        ...
        Eigen::RowVectorXf xsm = x_coord-xzeta;
        ...
        Eigen::RowVectorXcf g = -j1 * k0 * beta *(
            (((-j1 * k0 * r.array()).exp()).array())/(4.0 * M_PI * r.array())) *
            (((j1 * k0 * r).array()).inverse() + 1.0) *
            (xsm.array() / r.array()) * jacobian;
        // Integrate
        gfield(j) = g * Nweights;
    }// end of loop
    ux_scat = gfield * ps;
